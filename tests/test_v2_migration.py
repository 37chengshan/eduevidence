...
def test_migration_preserves_v1_ids(tmp_path):
    ...
    source_pack = json.loads((pack / "result.json").read_text(encoding="utf-8"))
    ...
    migrated_source_ids = {s["source_id"] for s in sources}
    migrated_study_ids = {s["study_id"] for s in studies}
    expected_source_ids = {s["source_id"] for s in source_pack["sources"]}
    expected_study_ids = {
        e["study_id"] for e in source_pack["evidence"] if e.get("study_id")
    }
    assert expected_source_ids <= migrated_source_ids
    assert expected_study_ids <= migrated_study_ids
    assert len(sources) >= len(expected_source_ids)
    assert len(studies) >= len(expected_study_ids)
...
def test_migration_maps_semantics(tmp_path):
    ...
    v1_by_finding = {f"FND-{eid}": ev for eid, ev in v1_ev.items()}
    ...
    for link in links:
        ...
        legacy = v1_by_finding.get(link["finding_id"])
        if legacy and legacy.get("decision_relation"):
            # An explicit V1 decision implication is research state and must be
            # preserved verbatim; relation_to_claim must not overwrite it.
            assert link["decision_implication"] == legacy["decision_relation"]
        elif link["relation_to_claim"] == "support":
...
def test_legacy_evidence_without_study_gets_explicit_unresolved_study(tmp_path):
    ...
    first_source_id = data["sources"][0]["source_id"]
    data["evidence"].append({
        "evidence_id": "E-LEGACY", "source_id": first_source_id,
