/**
 * web/js/dashboard.js — Upgraded Dashboard & Analytics Matrix
 */
...
export function renderDashboard() {
  const d = state.stats || {};
  const kpis = [
    { label: "纳入实证课题", value: d.total_projects || state.projects.length, sub: "动态扫描 result.json 契约" },
    { label: "证据条目 (Evidence)", value: d.total_evidence || 0, sub: "Claim-level 证据实体" },
    { label: "量化效应量 (Effect Sizes)", value: d.total_effect_sizes || 0, sub: "Hedges' g 效应值" },
    { label: "SSOT 图谱节点", value: d.total_nodes || 0, sub: "跨课题全局因果网络" }
  ];
...
function renderVerdictChart() {
  ...
  c.setOption({
    ...
    xAxis: { type: "category", data: ["PILOT", "ADOPT", "REJECT", "证据不足"], axisLabel: { color: textColor(), fontSize: 11, fontFamily: 'Inter, sans-serif' } },
    yAxis: { type: "value", minInterval: 1, axisLabel: { color: textColor(), fontFamily: 'Inter, sans-serif' }, splitLine: { lineStyle: { type: 'dashed', color: 'rgba(150,150,150,0.15)' } } },
    series: [{
      type: "bar",
      barWidth: '45%',
      data: [
        { value: counts.pilot, itemStyle: { color: '#f24d29', borderRadius: [4,4,0,0] } },
        ...
        { value: counts.insufficient + counts.other, itemStyle: { color: "#94a3b8", borderRadius: [4,4,0,0] } }
      ]
    }]
  }, true);
}
  ...
  c.setOption({
    ...
    series: [{
      ...
      data: [
        ...
        { value: ct, name: "反驳", itemStyle: { color: "#f24d29" } },
        { value: n, name: "中性", itemStyle: { color: "#94a3b8" } }
      ]
    }]
  }, true);
}
...
function renderCrossChart() {
  ...
  c.setOption({
    backgroundColor: "transparent",
    tooltip: { trigger: "axis" },
    legend: { data: ["证据条目", "数值效应量"], top: 0, textStyle: { color: textColor(), fontSize: 11, fontFamily: 'Inter, sans-serif' } },
    grid: { left: 40, right: 20, top: 35, bottom: 44 },
    xAxis: { type: "category", data: labels, axisLabel: { color: textColor(), fontSize: 10, rotate: 25, fontFamily: 'Inter, sans-serif' } },
    yAxis: { type: "value", minInterval: 1, axisLabel: { color: textColor(), fontFamily: 'Inter, sans-serif' }, splitLine: { lineStyle: { type: 'dashed', color: 'rgba(150,150,150,0.15)' } } },
    series: [
      { name: "证据条目", type: "bar", barGap: '20%', data: state.projects.map(p => p.evidence_count || 0), itemStyle: { color: '#f24d29', borderRadius: [3,3,0,0] } },
...
function renderProjCards() {
  $("#proj-cards").innerHTML = state.projects.map((p, i) => {
    ...
    if (p.mean_effect_size != null) {
      ...
      bars = `<div class="mini-bars"><div class="mini-bar" style="height:${Math.max(8, Math.min(100, Math.abs(v) * 100))}%;${v >= 0 ? "" : " background:var(--crimson);"}" title="mean g=${v}"></div></div>`;
    } else {
      bars = `<div class="mini-bars">
        <div class="mini-bar" style="height:${Math.min(100, 20 + dc.support * 3)}%;background:var(--emerald);" title="支持 ${dc.support}"></div>
        <div class="mini-bar" style="height:${Math.min(100, 20 + dc.contradict * 3)}%;background:var(--crimson);" title="反驳 ${dc.contradict}"></div>
...
export function renderMatrix() {
  $("#dash-table tbody").innerHTML = state.projects.map(p => {
    ...
    return `<tr>
      ...
      <td style="color: var(--text-muted);"><span style="color:var(--emerald)">${dc.support}</span> / <span style="color:var(--crimson)">${dc.contradict}</span> / ${dc.neutral}</td>
      <td style="font-family: var(--font-mono); font-weight: 600;">${esc(g)}</td>
