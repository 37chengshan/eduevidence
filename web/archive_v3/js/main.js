...
function renderReportList() {
  ...
  list.innerHTML = withReport.map(p => 
    `<div class="report-item" data-id="${p.id}">
      ...
      <div class="r-meta">${p.verdict || "—"} · ${p.evidence_count || 0} 条证据</div>
    </div>`
  ).join("");
...
function loadProjects() {
  Promise.all([api("/api/projects"), api("/api/labels")]).then(pair => {
    ...
    $("#dash-kpis").innerHTML = `<div class="empty-state">加载失败: ${err.message}</div>`;
  });
}
...
function init() {
  ...
  function triggerLandingTransition(e) {
    e.preventDefault();

    let overlay = document.getElementById("console-return-overlay");
    if (!overlay) {
      overlay = document.createElement("div");
      overlay.id = "console-return-overlay";
      overlay.style.cssText = `
        position: fixed;
        inset: 0;
        pointer-events: none;
        z-index: 99999;
        background: linear-gradient(270deg, #FAF9F5 0%, #F3F1EC 85%, #d97757 100%);
        box-shadow: -15px 0 40px rgba(217, 119, 87, 0.35);
    ...
    requestAnimationFrame(() => {
      overlay.style.transition = 'transform 0.48s cubic-bezier(0.22, 1, 0.36, 1)';
  ...
  $("#btn-back-to-landing")?.addEventListener("click", triggerLandingTransition);
  $("#topbar-to-landing")?.addEventListener("click", triggerLandingTransition);
