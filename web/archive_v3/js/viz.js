...
function pooledMean(forest) {
  ...
  forest.forEach(f => {
    ...
    const w = ci > 0 ? 3.84 / (ci * ci) : (Number(f.weight) || 1);
    if (isFinite(g)) { num += w * g; den += w; }
  });
...
export function renderForest(forest) {
  ...
  sorted.forEach((f, idx) => {
    // Label with WWC rating if available
    let label = f.study_label || `Study ${idx + 1}`;
    if (f.wwc_rating) label += ` [${f.wwc_rating}]`;
    
    yAxisData.push(label);
    const g = Number(f.effect_size) || 0;
    const ciLo = Number(f.ci_lower) || g;
    const ciHi = Number(f.ci_upper) || g;
    
    scatterData.push({
      value: [g, idx],
      itemStyle: { color: g >= 0 ? '#10B981' : '#EF4444' },
      info: f
    });
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        return `<div style="font-size:0.8rem;">
          ...
          效应量(g): ${Number(f.effect_size).toFixed(2)} [${Number(f.ci_lower).toFixed(2)}, ${Number(f.ci_upper).toFixed(2)}]<br/>
