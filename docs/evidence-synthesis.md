...
3. **不进行数值合并（no pooled effect size）**：教育证据研究间人群、干预、
   测量异质性大，强行 meta 合并会产生虚假精确性。v4 以方向一致性与独立研究
   覆盖代替点估计；"多大把握"的定量表述交给规则化置信度公式
   （0.30 质量 + 0.25 一致性 + 0.20 直接性 + 0.25 独立研究数 − 冲突罚 − 未支持罚）。
...
1. **leave-one-study-out 敏感性**：逐次剔除单一独立研究后重算方向判定与置信度；
   若结论方向翻转，verdict 标注为对单点证据敏感（v4 进行中，随
   docs/evidence-synthesis.md 配套脚本落地）。
