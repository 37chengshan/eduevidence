...
- **画面**：04 EvidenceFlow 协议 SVG + 09 干预时间线 + 10 评价流程 → 头部主题切换 → 12 来源与溯源。
- **操作**：快速滚动展示 3 张信息图；回到头部，依次点击 Academic / Presentation 主题（观察配色切换）；滚动到 12，展示来源表与 Fetch 溯源。
- **旁白**："研究流程、裁决、干预、评价全部有统一风格的信息图，离线可用、不依赖任何 CDN。五种主题一键切换。最后是来源与抓取溯源——每条证据的来源、获取方式、时间都可查。"
- **验证点**：4 张信息图内嵌 SVG 渲染正常；主题切换改变 `data-theme` 且 localStorage 持久化；12 节来源表 3 行 + Fetch 溯源表头完整。
